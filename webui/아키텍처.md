...
- **접속 URL**: https://d1opjo4xb8q213.cloudfront.net
...
flowchart TB
    ...
    subgraph Global["CloudFront (유일한 진입점)"]
        CF["Distribution E2ZOFV24VJXTC8<br/>d1opjo4xb8q213.cloudfront.net"]
    end
...
- **동시 실행 3건 제한**: 비용 폭주 방지 (Lambda가 실행 생성 시 검사, `MAX_ACTIVE_RUNS`).
...
1. https://d1opjo4xb8q213.cloudfront.net 접속 → 가족 계정으로 로그인
...
curl -s -X POST https://d1opjo4xb8q213.cloudfront.net/api/runs \
  -H "x-access-token: $TOKEN" -H "content-type: application/json" \
  -H "x-amz-content-sha256: $HASH" -d "$BODY"

# 3. 상태 조회 / 보고서
curl -s -H "x-access-token: $TOKEN" https://d1opjo4xb8q213.cloudfront.net/api/runs
curl -s -H "x-access-token: $TOKEN" "https://d1opjo4xb8q213.cloudfront.net/api/runs/<run_id>/report?name=complete_report.md"
```
