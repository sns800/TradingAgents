...
#
# 배경: 하류 에이전트는 원본 도구 데이터에 접근하지 못하기 때문에(중기 #6
# 이전에는 Msg Clear가 파기했고, 병렬화 이후에는 분석가 전용 채널을 하류가
# 읽지 않음), 기존에는 하류 에이전트가 인용한 수치가 실제 데이터와 맞는지
# 잡을 기준점이 전혀 없었습니다. 스냅샷이 상태 필드로 보존되면서 이 감사가
# 가능해졌습니다.
#
# 설계 원칙:
